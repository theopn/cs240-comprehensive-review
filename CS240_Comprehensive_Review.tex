...
\begin{itemize}
    ...
    \begin{itemize}
        \item You cannot assign something new to array unlike ptr.
        \begin{lstlisting}[style=CStyle]
            ptr = arr; /* allowed */
            arr = ptr; /* NOT allowed */
        \end{lstlisting}
        \item Array definition allocates memory for every elements in the array, whereas you need to dynamically allocate memory for a var that ptr points to (ptr itself is allocated; 8 bytes).
    \end{itemize}
\end{itemize}
...
\begin{lstlisting}[style=CStyle]
    ...
    ptr += 3; /* Points 6th element - 0 - Remember uninitialized elements are 0 */
\end{lstlisting}
